...
p_load(terra, tidyterra, exactextractr, sf, dplyr, fastDummies, viridis, ggplot2)
...
```{r}
# save 300m prediction grid as raster 
pred_rast <- imp_agg

# save 300m prediction grid as vector
pred_vect <- as.points(pred_rast)
plot(pred_vect, y="NC_imperviousness_2019")
```
...
#### Extraction example on a spatial subset 
...
Create NLCD dataframe with all information

```{r}
nlcd_classes <- list(value = c(11, 21, 22, 23, 24, 31, 41, 42, 43, 52,
                               71, 81, 82, 90, 95),
                     class = c("WTR", "OSD", "LID", "MID", "HID", 
                                     "BRN", "DFO", "EFO", "MFO", "SHB", 
                                     "GRS", "PAS", "CRP", "WDW", "EHW"),
                     names = c("Open Water", 
                     ...
                     col = c("#476ba1", "#decaca", "#d99482", "#ee0000",
...
Function to compute NLCD class ratio per buffer (150m-radius circle) 
...
compute_ndld_class_ratio0 <- function(spvect, nlcd, width = 150) {
  ...
  nlcd_cropped_df <- as.data.frame(nlcd_cropped, xy = T) 
  ...
  new_spvect <- new_spvect[names(new_spvect)[grepl("class_",
                                                   names(new_spvect))]]
...
pred_nlcd_ratio_df <- cbind(pred_nlcd_ratio_df[, c("x", "y")], 
                            pred_nlcd_ratio_df[,
