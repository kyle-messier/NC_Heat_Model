...
test_that("Check create_sets_rnds works", {
  ...
  expect_equal(class(obs), class(train))
  expect_equal(colnames(obs), colnames(test))
  expect_equal(sum(unique(train$station) %in% unique(test$station)),
               0) # means all stations are different in each sets
...
test_that("Check create_sets_t works", {
  ...
  expect_equal(class(obs), class(train))
  expect_equal(colnames(obs), colnames(test))
  expect_equal(sum(unique(train$date) %in% unique(test$date)),
               0) # means all dates are different in each sets
...
test_that("Check create_sets_s works", {
  ...
  expect_error(create_sets_s(obs, test_counties), 
  ...
  expect_warning(create_sets_s(obs[county %in% test_counties, ], test_counties),
                 "train sample is empty")
  ...
  expect_equal(class(obs), class(train))
  expect_equal(colnames(obs), colnames(test))
  expect_equal(sum(unique(train$county) %in% unique(test$county)),
               0) # means all counties are different in each sets
...
test_that("Check create_sets_net works", {
  ...
  obs <- fread(paste0("../input/",
                      ...
                      "20220601-20220831", 
                      "-space-time-covariates.csv"))
  expect_error(create_sets_net(obs, test_net), 
  ...
  # -- 5th example: training set is empty 
  test_net <- c("ECONET")
  expect_warning(create_sets_net(obs[network %in% test_net, ], test_net),
                 "train sample is empty")
  ...
  expect_equal(class(obs), class(train))
  expect_equal(colnames(obs), colnames(test))
  expect_equal(sum(unique(train$network) %in% unique(test$network)),
               0) # means all counties are different in each sets
