...
for (i in 2:7){
  pred_rastds_era5[[i-1]] <- terra::project(new_pred_vect_rtp, crs(era5_rastds[[i]])) %>%
...
plot(pred_rastds_era5$TNwmo$`2022-05-07`)
```
### TODEL
...
pred_rastds_era5 = list()
for (i in 2:7){
  pred_rastds_era5[[i-1]] <- terra::project(new_pred_vect_rtp,
                                            crs(era5_rastds[[i]])) %>%
    terra::extract(x = era5_rastds[[i]], y = ., bind = T) %>%
    terra::project(., crs(new_pred_vect_rtp)) %>%
